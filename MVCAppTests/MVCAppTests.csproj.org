...
<Project ToolsVersion="14.0" DefaultTargets="CodeCoverageReport" xmlns="http://schemas.microsoft.com/developer/msbuild/2003">
<UsingTask AssemblyFile="$(MSBuildStartupDirectory)\MVCApp\packages\MSBuild.Extension.Pack.1.9.1\tools\net40\MSBuild.ExtensionPack.dll" TaskName="Nunit3"  />
  ...
  <Target Name="Test">
    ...
    <NUnit3 Assemblies="@(TestAssembly)" ToolPath="$(MSBuildStartupDirectory)\MVCApp\packages\NUnit.ConsoleRunner.3.7.0\tools" ContinueOnError="false" OutputXmlFile="$(NUnitResultsFile)" />
  </Target>

  <Target Name="TestResultsReport" DependsOnTargets="Test">
  
      <Message Text="TestResultsReport Reportunit Calling ,Creating unit testing report  ReportUnit.exe $(MSBuildStartupDirectory)$(NUnitResultsFolder) $(MSBuildStartupDirectory)\UnitTestReport" />
      <Message Text="deleting reports"	 />
   <Exec Command="C:\Projects\ASP.NET-master\MVCDOTNET\MVCApp\packages\ReportUnit.1.2.1\tools\ReportUnit.exe &quot;$(MSBuildStartupDirectory)\$(NUnitResultsFolder)&quot; &quot;$(MSBuildStartupDirectory)\UnitTestReport&quot;"   />
   ...
   <Target Name="CoverageOC" DependsOnTargets="TestResultsReport">
  	...
  	<Exec Command="C:\Projects\ASP.NET-master\MVCDOTNET\MVCApp\packages\OpenCover.4.6.519\tools\OpenCover.Console.exe -register:Path64 -returntargetcode -target:&quot;C:\Projects\ASP.NET-master\MVCDOTNET\MVCApp\packages\NUnit.ConsoleRunner.3.7.0\tools\nunit3-console.exe&quot; -targetargs:&quot;@(TestAssembly) /version /trace=Debug /config=Debug /result=$(MSBuildStartupDirectory)\$(NUnitResultsFolder)\TestResult.xml&quot;    -filter:&quot;+[*.*]* +[*Test]*&quot; -output:&quot;$(MSBuildStartupDirectory)\CoverageResults\coverage-results.xml&quot;"  />
    ...
    <Target Name="CodeCoverageReport" DependsOnTargets="CoverageOC">
  	...
  	<ReportGenerator ReportFiles="$(MSBuildStartupDirectory)\$(CodeCoverageResultsFolder)\*.xml" TargetDirectory="$(MSBuildStartupDirectory)\$(CodeCoverageReportFolder)" ReportTypes="Html" SourceDirectories="$(SourceDirectories)" HistoryDirectory="$(CoverageHistoryDirectory)" AssemblyFilters="+Include;-Excluded" VerbosityLevel="Verbose" />
